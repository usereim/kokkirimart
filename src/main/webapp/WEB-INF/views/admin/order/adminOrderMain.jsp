				...
				<tr>
				  <td>
				    <select name="s_search_type" disabled >
						...
						<option value="member_name">주문자이름</option>
						<option value="order_name">주문자아이디</option>
						<option value="order_hp1">주문자휴대폰번호</option>
						<option value="product_name">주문상품품명</option>
					</select>
          ...
          <c:when test="${item.order_id != pre_order_id }">  
            <c:choose>
              ...
              <c:otherwise>
				...
				<td width=20%>
				 <strong>${item.pay_date }</strong> 
				</td>
				<td width=50% align=left >
				    <strong>주문자:${item.member_name}</strong><br>
				  <strong>주문자 번화번호:${item.order_tel1}-${item.order_tel2}-${item.order_tel3}</strong><br>
				  <strong>수령자:${item.order_name}</strong><br>
				  <strong>수령자 번화번호:${item.order_hp1}-${item.order_hp2}-${item.order_hp3}</strong><br>
				  <strong>주문상품명(수량):${item.product_name}(${item.order_qty})</strong><br>
				     <c:forEach var="item2" items="${newOrderList}" varStatus="j">
				       <c:if test="${j.index > i.index }" >
				          <c:if  test="${item.order_id ==item2.order_id}" >
				            <strong>주문상품명(수량):${item2.product_name}(${item2.order_qty})</strong><br>
				      </c:if>   
				       </c:if>
				    </c:forEach> 
				</td>
